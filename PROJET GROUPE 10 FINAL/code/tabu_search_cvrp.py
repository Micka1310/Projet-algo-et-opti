...
class TabuSearch_CVRP:
    """
    Classe implémentant l'algorithme Tabu Search pour le problème CVRP
    """

    def __init__(self, instance_vrplib: Dict, initial_solution: List[List[int]]):
        """
        Initialisation de l'algorithme Tabu Search
        ...
        # Liste taboue (stocke les mouvements interdits)
        self.tabu_list = []
        self.tabu_tenure = 10  # Durée de vie d'un mouvement dans la liste taboue
    ...
    def generate_neighborhood(self, solution: List[List[int]],
                            neighborhood_size: int = 50) -> List[Tuple]:
        """
        Génère un échantillon du voisinage de la solution
        ...
        for route_idx, route in enumerate(solution):
            ...
            num_swaps = min(5, (len(route) - 2) * (len(route) - 3) // 2)
        ...
        for r1_idx in range(len(solution)):
            for r2_idx in range(r1_idx + 1, len(solution)):
                ...
                # Limiter les essais
                num_swaps = min(3, (len(route1) - 2) * (len(route2) - 2))
                for _ in range(num_swaps):
        ...
        for route_idx, route in enumerate(solution):
            ...
            num_2opts = min(3, (len(route) - 2) * (len(route) - 3) // 2)
    ...
    def run(self, max_iterations: int = 3000, tabu_tenure: int = 10,
            neighborhood_size: int = 50, diversification_freq: int = 100) -> Dict:
        """
        Exécute l'algorithme Tabu Search
        ...
        """
        print(f"\n{'='*60}")
        print(f"Démarrage de l'algorithme Tabu Search pour CVRP")
        print(f"{'='*60}")
    ...
    def diversify_solution(self):
        """Applique une perturbation pour diversifier la recherche"""
        # Effectuer plusieurs mouvements aléatoires
        num_perturbations = random.randint(3, 7)
    ...
    def plot_convergence(self, save_path: str = None):
        ...
        plt.title('Convergence de l\'algorithme Tabu Search pour CVRP')
        plt.legend()
