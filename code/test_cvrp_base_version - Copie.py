...
while clients_remaining :
    ...
    initial_solution1.append(current_route) # pour la solution initiale sans dépôt
...
def compute_distance_matrix(coords):
    # S'assurer que coords est un tableau numpy pour permettre la soustraction
...
def random_removal(solution, n_remove=3):
    all_nodes = [(r_idx, node) for r_idx, route in enumerate(solution) for node in route]
    to_remove = random.sample(all_nodes, min(n_remove, len(all_nodes)))
    new_sol = [route[:] for route in solution]
    removed = []
    for r_idx, node in sorted(to_remove, reverse=True):
        new_sol[r_idx].remove(node)
        removed.append(node)
    return new_sol, removed

def greedy_insertion_repair(partial_sol, removed, instance):
    ...
    for node in removed:
        ...
        best_pos = None
        best_cost = float('inf')
        for r_idx, route in enumerate(partial_sol):
            if sum(demands[i] for i in route) + demands[node] <= capacity:
                for pos in range(len(route) + 1):
                    new_route = route[:pos] + [node] + route[pos:]
                    dummy = partial_sol[:r_idx] + [new_route] + partial_sol[r_idx+1:]
                    cost = evaluate_solution(dummy, demands, capacity, dist_matrix)
                    if cost < best_cost:
                        best_cost = cost
                        best_route = r_idx
                        best_pos = pos
        if best_route is not None:
            partial_sol[best_route].insert(best_pos, node)
        else:
            partial_sol.append([node])
    return partial_sol

def solve_vrp(instance, time_limit=60, seed=42):
    random.seed(seed)
    coords = instance["node_coord"]
    demands = instance["demand"]
    capacity = instance["capacity"]
    ...
    while time.time() - start < time_limit:
        partial, removed = random_removal(current_sol, n_remove=5)
        new_sol = greedy_insertion_repair(partial, removed, instance)
        new_cost = evaluate_solution(new_sol, demands, capacity, dist_matrix)
        if new_cost <= current_cost:
            current_sol = new_sol
            current_cost = new_cost
            if new_cost < best_cost:
                best_sol = [r[:] for r in new_sol]
                best_cost = new_cost
    return {"routes": best_sol, "cost": best_cost}

final_solution_optimized = solve_vrp(instance_vrplib, time_limit=45, seed=42)
...
# Personnalisation des légendes
plt.title(f"Solution de l'instance VRP : {instance_vrplib['name']} au coût totale {final_solution_optimized['cost']}")
plt.xlabel("Coordonnée X")
plt.ylabel("Coordonnée Y")
plt.legend()
plt.axis('equal') # assure une échelle correcte
